-- ============================================================================
-- UPDATE ENQUEUE_JOBS FOR JOB_TYPE
-- ============================================================================
-- NEW MIGRATION (not modifying 021): Adds job_type parameter to enqueue_jobs.
-- Keeps function generic - caller must pass correct source_id.
-- Updates ON CONFLICT to use (intake_id, source_id, job_type) matching unique index.
...
CREATE OR REPLACE FUNCTION enqueue_jobs(
  ...
  p_job_type TEXT DEFAULT 'pricing'
)
RETURNS INTEGER AS $$
DECLARE
  v_source_id UUID;
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      ON CONFLICT (intake_id, source_id, job_type) 
...
COMMENT ON FUNCTION enqueue_jobs(UUID, UUID[], INTEGER, INTEGER, TEXT) IS 
  'Enqueues jobs for a given intake and source IDs with staggered scheduling. Generic function - caller must pass correct source_id. Returns count of jobs inserted.';
